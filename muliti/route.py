...
def doRouter(connection_data):
    ...
    try:
        ...
        cli_response = net_connect.send_command("sh ip int bri | i down ")
        cli_response = cli_response.replace("Interfaces down: ", "")
        if cli_response != "":
            print(str(
                ip_address) + " - " + bcolors.WARNING + "Some interfaces are down.  " + cli_response + " " + bcolors.ENDC)
            config_ok = False

        cli_response2 = net_connect.send_command("sh run | i ip route 8.8.8.8 255.255.255.255")
        if cli_response2.strip() == "":
            print(str(
                ip_address) + " - " + bcolors.WARNING + "WARNING - couldn't find static route to 8.8.8.8" + bcolors.ENDC)
            config_ok = False

        ip_next_hop = ""
        if cli_response2 != "":
            ip_next_hop = cli_response2.split(" ")[5]

        if ip_next_hop == "":
            print(str(
                ip_address) + " - " + bcolors.WARNING + "WARNING - couldn't find next-hop IP address " + bcolors.ENDC)
            config_ok = False
        ...
        if config_ok:
            ...
            print(str(
                ip_address) + " - " + bcolors.FAIL + "Routes weren't added because config is incorrect" + bcolors.ENDC)
            print("gateway -", ip_next_hop)
            return ip_address
